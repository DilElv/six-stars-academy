      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => exportLedgerToCSV(items, { type: 'pemasukan', month, year })} title="Export CSV" className="flex items-center gap-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-navy-700 text-xs font-semibold px-3 py-2 rounded-xl"><FileSpreadsheet size={14} className="text-emerald-600" /> CSV</button>
            <button onClick={() => exportLedgerToPDF(items, { type: 'pemasukan', month, year, total })} title="Export PDF" className="flex items-center gap-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-navy-700 text-xs font-semibold px-3 py-2 rounded-xl"><FileText size={14} className="text-red-600" /> PDF</button>
          </div>
        </div>
      </div>

      <div className="grid lg:grid-cols-3 gap-4">
        <div className="rounded-3xl p-5 flex flex-col justify-center bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-lg shadow-emerald-500/20">
        ...
        <div className="glass-card rounded-3xl p-4 sm:p-5 lg:col-span-2">
          ...
          <DailyLedgerChart items={items} daysInMonth={daysInMonth} color="#10b981" gradientId="pemasukanGradient" height={180} />
        </div>
      </div>

      <div className="glass-card rounded-3xl p-4 sm:p-5">
        <div className="text-xs font-semibold text-gray-500 mb-2">Sumber Pemasukan — {MONTH_LABELS[month]} {year}</div>
